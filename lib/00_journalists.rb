...
def number (journalists)
    n = journalists.length
    puts "Il y a #{n} dans cette array."
end

# Quel est le handle le plus court de cette liste ?
def minimum (ar)
    smallest = ar.min {|x,y| x.size <=> y.size}
    puts "L'handle le plus court est #{smallest}."
end
...
def five (handles)
    ...
    puts "Il y a #{coucou.size} handles qui contient 5 caractères."
end
...
def maj (handles)
    ...
    puts "Il y a #{majs.length} handles qui commencent par une majuscule."
end

# Trie la liste de handle par ordre alphabétique.
def alpha (handles)
    handles.map(&:downcase)
    puts "Voici l'ordre alphabétique #{handles.sort}"
end

# Trie la liste de handle par taille des handles.
def taille (handles)
